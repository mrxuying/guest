...
def add_event(request):
    ...
    if result:
        return JsonResponse({'status':10023,'message':'evnet name already exists'})
    ...
    return JsonResponse({'status':200,'message':'add evnet success'})
...
def add_guest(request):
    ...
    if n_time >= etime:
        return JsonResponse({'status':10025,'message':'event has stared'})
    try:
